...
func (a *App) AddReaction(messageID, emoji, username, channel string) error {
	...
	err = UpdateMessage(channel, *foundMsg)
	if err != nil {
	...
	log.Printf("👍 %s %s реакцию %s на сообщение в #%s",
		username,
		map[bool]string{true: "убрал", false: "добавил"}[alreadyReacted],
		emoji,
		channel)
...
func (a *App) DeleteChannel(name, username string) error {
	...
	systemChannels := map[string]bool{
		"general":  true,
		"random":   true,
		"dev-team": true,
	}
